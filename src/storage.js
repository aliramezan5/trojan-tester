...
export async function getSetting(key, fallback = null) {
  try {
    ...
    const value = await requestAsPromise(tx.objectStore(STORES.settings).get(key));
    db.close();
...
export async function getCachedResult(fingerprintId) {
  ...
  try {
    ...
    const entry = await requestAsPromise(tx.objectStore(STORES.cache).get(fingerprintId));
    db.close();
...
export async function getHistoryRuns(limit = 20) {
  try {
    ...
    const all = await requestAsPromise(tx.objectStore(STORES.history).getAll());
    db.close();
